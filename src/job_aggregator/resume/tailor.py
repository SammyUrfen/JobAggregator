...
def _select_prompt(
    ...
    system = (
        ...
        "and debug before release). Name only capabilities that a project or a skill can prove. "
        "Skip traits that no résumé line proves, such as 'comfortable with ambiguity'. When the "
        "job names a tool or practice it expects in daily work, that is a need.\n"
        "\n"
        ...
        "rolls back a victim transaction') over the category ('concurrency control'). Write "
        "without 'I', 'my' or 'we'. Each bullet is one or two sentences and at most "
        f"{_MAX_BULLET_WORDS} words. Leave out detail this job does not need. Do not pack three "
        "mechanisms into one bullet.\n"
        "\n"
